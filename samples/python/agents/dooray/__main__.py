...
from agents.dooray.agent import DoorayAgent
...
def create_skills_from_mcp(agent: DoorayAgent) -> list[AgentSkill]:
    skills = []
...
async def async_main(host, port):
  try:
    ...
    agent_card = AgentCard(
      name='Dooray Agent',
      description='Helps Dooray-based scheduling or messaging tasks',
      url=f'http://{host}:{port}/',
      version='1.0.0',
      defaultInputModes=DoorayAgent.SUPPORTED_CONTENT_TYPES,
      defaultOutputModes=DoorayAgent.SUPPORTED_CONTENT_TYPES,
    ...
    logger.info(f'🚀 Starting Dooray Agent server at http://{host}:{port}')
    server.start()
