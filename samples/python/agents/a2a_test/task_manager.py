...
from agent import DockerEvalAgent
...
class AgentTaskManager(InMemoryTaskManager):
  ...
  def __init__(self, agent: DockerEvalAgent):
    super().__init__()
    self.agent = agent
  ...
  async def on_send_task_subscribe(self, request: SendTaskStreamingRequest) -> AsyncIterable[SendTaskStreamingResponse] | JSONRPCResponse :
    print("AgentTaskManager: on_send_task_subscribe")
  ...
  async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
    print("AgentTaskManager: on_send_task")
    error = self._validate_request(request)
    if error:
      return error
    await self.upsert_task(request.params)
  ...
  def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> None:
    print("AgentTaskManager: _validate_request")
    task_send_params: TaskSendParams = request.params

    if not utils.are_modalities_compatible(
      task_send_params.acceptedOutputModes, DockerEvalAgent.SUPPORTED_CONTENT_TYPES
    ):
      logger.warning(
        "Unsupported output mode. Received %s, Support %s",
        task_send_params.acceptedOutputModes,
        DockerEvalAgent.SUPPORTED_CONTENT_TYPES,
      )
  ...
  async def _stream_generator(self, request:SendTaskStreamingRequest) -> AsyncIterable[SendTaskStreamingResponse] | JSONRPCResponse :
    print("AgentTaskManager: _stream_generator")
    task_send_params: TaskSendParams = request.params
    query = self._get_user_query(task_send_params)

    try:
      async for item in self.agent.stream(query, task_send_params.sessionId):
        print("AgentTaskManager: stream_generator: item")
  ...
  async def _invoke(self, request: SendTaskRequest) -> SendTaskResponse:
    print("AgentTaskManager: _invoke")
    task_send_params: TaskSendParams = request.params
    query = self._get_user_query(task_send_params)
    try:
  ...
  async def _update_store(self, task_id: str, status: TaskStatus, artifacts: list[Artifact]) -> Task:
    print("AgentTaskManager: _update_store")
  ...
  def _get_user_query(self, task_send_params: TaskSendParams) -> str:
    print("AgentTaskManager: _get_user_query")
