...
class ConversationClient:
    ...
    async def _send_request(self, request: JSONRPCRequest) -> dict[str, Any]:
        url = self.base_url.rstrip('/') + '/' + request.method
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                # 응답 본문까지 찍어서, 서버가 무슨 오류 메시지를 보내는지 보자
                ...
                print("[ERROR] JSON decode failed. response.text =", response.text)
    ...
    async def list_agents(self, payload: ListAgentRequest) -> ListAgentResponse:
        ...
        print("[DEBUG:list_agents] raw response →", raw.get("result", []))
        return ListAgentResponse(**raw)
