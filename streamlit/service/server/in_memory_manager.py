    ...
    def get_pending_messages(self) -> list[tuple[str, str]]:
        ...
        for message_id in self._pending_message_ids:
            if message_id in self._task_map:
                ...
                task = next(filter(lambda t: t.id == task_id, self._tasks), None)
                if not task:
                    rval.append((message_id, ''))
                    continue

                # task.history 가 최소한 하나 있고, 텍스트 파트가 있다면
                last = task.history[-1]
                if last.parts:
                    p = last.parts[0]
                    text = p.text if (p.type == 'text') else 'Working...'
                    rval.append((message_id, text))
                    continue

            # 맵핑된 태스크가 없으면 빈 상태 문자열
            rval.append((message_id, ''))
