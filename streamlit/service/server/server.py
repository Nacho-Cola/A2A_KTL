...
class ConversationServer:
    ...
    def __init__(self, router: APIRouter, manager: ApplicationManager | None = None,):
    ...
    async def _send_message(self, request: Request):
        ...
        # metadata가 없으면 빈 dict로 초기화
        metadata = params.get('metadata') or {}
        # conversation_id 유무에 따라 동작 분기(필수라면 에러, 선택이라면 default 처리)
        if not metadata.get('conversation_id'):
            # 1) 필수로 쓴다면 에러를 던지거나
            # raise HTTPException(status_code=400, detail="conversation_id is required")
            # 2) 선택사항이라면, sanitize_message 에서 채워지도록 그냥 빈 메타 전달
            metadata = {}
 
        message = Message(
            role=params.get('role', 'user'),
            parts=[Part(**p) for p in params.get('parts', [])],
            metadata=metadata
        )
    ...
    async def _unregister_agent(self, request: Request):
        ...
        url = data['params']
        # ADKHostManager 에 unregister_agent 가 구현되어 있어야 합니다.
        try:
            self.manager.unregister_agent(url)
            return UnregisterAgentResponse()
        except AttributeError:
            return UnregisterAgentResponse(error="unregister not supported")
        except Exception as e:
            return UnregisterAgentResponse(error=str(e))
